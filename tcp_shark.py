...
def process2():
	...
	subprocess.run(["sudo", "tcpdump", "-c", packets, "-w", file + ".pcap"])
	input('\nPresione Enter para regresar al menú principal...')

def process3():
	interface = input('Ingrese el nombre de la interfaz: ')
	subprocess.run(["sudo", "tcpdump", "-i", interface])
	input('\nPresione Enter para regresar al menú principal...')

def process4():
	interface = input('Ingrese el nombre de la interfaz: ')
	protocol = input('Ingrese el protocolo a capturar (tcp, udp, icmp, etc.): ')
	subprocess.run(["sudo", "tcpdump", "-i", interface, protocol])
	input('\nPresione Enter para regresar al menú principal...')

def process5():
	subprocess.run(["sudo", "tcpdump", "-A"])
	input('\nPresione Enter para regresar al menú principal...')

def process6():
	subprocess.run(["sudo", "tcpdump", "-tttt"])
	input('\nPresione Enter para regresar al menú principal...')

def process7():
	file = input('Ingrese el nombre del archivo para guardar la captura(Sin extension): ')
	subprocess.run(["sudo", "tcpdump", "-w", file + ".pcap"])
	input('\nPresione Enter para regresar al menú principal...')

def process8():
	ip = input('Ingrese la dirección IP o el rango de red: ')
	subprocess.run(["sudo", "tcpdump", "host", ip])
	input('\nPresione Enter para regresar al menú principal...')

def process9():
	port = input('Ingrese el puerto o rango de puertos: ')
	subprocess.run(["sudo", "tcpdump", "port", port])
	input('\nPresione Enter para regresar al menú principal...')
...
def process12():
	...
	subprocess.run(["sudo", "tcpdump", "greater", size])
	input('\nPresione Enter para regresar al menú principal...')
